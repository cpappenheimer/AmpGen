...
template <class T>
void generateSource(T &pdf, const std::string &sourceFile, MinuitParameterSet &mps)
{
  ...
  phsp.setRandom(&rnd);
  INFO("Generating norm events");
  EventList_t normEvents = phsp.generate(nEvents);
  INFO("Generated norm events");
...
Particle getTopology(const pdf_t &pdf)
{
...
template <typename pdf_t>
std::vector<Particle> getDecayChains(const pdf_t &pdf)
{
  if constexpr (std::is_same<pdf_t, FixedLibPDF>::value)
  {
    FATAL("Cannot deduce decay topology from a compiled library, check generator options");
...
template <typename pdf_t>
void generateEvents(EventList &events, pdf_t &pdf, const phspTypes &phsp_type, const size_t &nEvents, const size_t &blockSize, TRandom *rndm, const bool &normalise = true)
{
...
int main(int argc, char **argv)
{
  ...
  if (sourceOnly)
  {
    ...
    {
      INFO("Creating CoherentSum PDF");
      CoherentSum pdf(eventType, MPS);
      INFO("Created CoherentSum PDF");
